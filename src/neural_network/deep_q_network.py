...
state_size = 33  # 32
action_size = 76 
path_to_weights = "./deep_q_network.h5" 


def build_new_model_and_load_weights(load_ws=False):
    model = Sequential()
...
def get_index_of_best_action_from_nn(model, state, selektovana_polja):
    state = np.reshape(state, [1,state_size])
    print(state)
    
    potencijalne_akcije = []
    
    for sp in selektovana_polja:
        state[0][state_size-2] = sp
        predict_values = model.predict(state)[0]
        index_of_action = np.argmax(predict_values)
        nova_potencijalna_akcija = (index_of_action, predict_values[index_of_action], sp)
